...
function playCelebrationSound() {
  ...
  try {
    ...
    const startSound = () => {
      ...
      // 3-second celebration trumpet fanfare, generated in-browser.
      const master = ctx.createGain();
      master.gain.setValueAtTime(0.0001, now);
      master.gain.exponentialRampToValueAtTime(0.85, now + 0.04);
      master.gain.setValueAtTime(0.85, now + 2.65);
      ...
      brassFilter.frequency.setValueAtTime(1350, now);
      brassFilter.Q.setValueAtTime(0.8, now);
      brassFilter.connect(master);

      function playTrumpetNote(freq, start, duration, volume = 0.18) {
        const gain = ctx.createGain();
        gain.gain.setValueAtTime(0.0001, start);
        gain.gain.exponentialRampToValueAtTime(volume, start + 0.035);
        gain.gain.setValueAtTime(volume * 0.85, start + Math.max(0.05, duration - 0.08));
        gain.gain.exponentialRampToValueAtTime(0.0001, start + duration);

        // Layered saw/triangle oscillators gives a brighter brass-like tone.
        [
          { type: "sawtooth", detune: -6, vol: 1 },
          { type: "sawtooth", detune: 7, vol: 0.75 },
          { type: "triangle", detune: 0, vol: 0.45 }
        ].forEach(layer => {
          ...
          layerGain.gain.setValueAtTime(layer.vol, start);
      ...
      function playDrumHit(start, duration = 0.12, volume = 0.08) {
      ...
      // Fanfare phrase: C major rise, repeat, then a held high finish.
      const C5 = 523.25;
      const E5 = 659.25;
      const G5 = 783.99;
      const C6 = 1046.5;
      ...
      playDrumHit(now + 0.00, 0.10, 0.07);
      playTrumpetNote(C5, now + 0.00, 0.26, 0.16);
      playTrumpetNote(E5, now + 0.28, 0.26, 0.16);
      playTrumpetNote(G5, now + 0.56, 0.32, 0.17);
      playTrumpetNote(C6, now + 0.92, 0.42, 0.19);

      playDrumHit(now + 1.36, 0.10, 0.07);
      playTrumpetNote(G5, now + 1.36, 0.24, 0.16);
      playTrumpetNote(C6, now + 1.62, 0.24, 0.18);
      playTrumpetNote(D6, now + 1.88, 0.26, 0.18);
      playTrumpetNote(E6, now + 2.16, 0.78, 0.2);

      // Add a lower harmony underneath the final note for a bigger finish.
      playTrumpetNote(C6, now + 2.16, 0.78, 0.12);
      playDrumHit(now + 2.12, 0.16, 0.08);
    };
