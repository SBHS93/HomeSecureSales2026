...
#sale-gif-card{
  ...
  border-radius: 16px;
  padding: 16px;
  width: min(1300px, 90vw);     /* ✅ forces it bigger */
...
// Put your sound file here (Netlify): /sounds/sale.wav
const SALE_SOUND_URL = "/sounds/sale.wav";

let soundEnabled = localStorage.getItem("soundEnabled") === "1";

function ensureSoundEnableButton() {
  ...
  btn.addEventListener("click", async () => {
    ...
    // Unlock audio playback for autoplay-restricted browsers
    try {
      const a = new Audio(SALE_SOUND_URL);
...
function playSaleSound() {
  ...
  try {
    ...
    audio.volume = 0.9; // 0.0 to 1.0
    audio.play().catch(() => {});
  } catch (_) {}
}

window.addEventListener("DOMContentLoaded", ensureSoundEnableButton);
