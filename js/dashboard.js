...
function getMondayStartUTC(now = new Date()) {
    ...
    d.setUTCDate(diff);
    d.setUTCHours(0, 0, 0, 0); // ✅ Monday 00:00:00.000 UTC
    return d;
}
...
function inCurrentWeek(dateStr) {
    // Ensure the sale is interpreted at UTC midnight for that date
    const saleDate = new Date(dateStr + "T00:00:00.000Z");

    const monday = getMondayStartUTC(new Date());

    const sundayEnd = new Date(monday);
    sundayEnd.setUTCDate(sundayEnd.getUTCDate() + 6);
    sundayEnd.setUTCHours(23, 59, 59, 999); // ✅ Sunday 23:59:59.999 UTC

    return saleDate >= monday && saleDate <= sundayEnd;
}
