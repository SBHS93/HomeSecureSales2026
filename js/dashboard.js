...
const SWEEP_TEAM = ["Bradley", "Dean", "Keith", "Ross", "Sudeep"];
...
const AGENT_SALE_GIFS = {
  Craig: "https://media3.giphy.com/media/v1.Y2lkPTc5MGI3NjExNmllNmd5YjFma2FudXV5bWl6aGRpM2xlaTNkd3Vjd3VpNG52NHhlbSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/65GiuFnyEuMjm/giphy.gif",
  ...
  Ross: "https://media.tenor.com/Rfx-bdFwV38AAAAM/purple-cobra.gif",
...
function getCelebrationAudioContext() {
  ...
  if (!celebrationAudioCtx) celebrationAudioCtx = new AudioContext();
  return celebrationAudioCtx;
}

function unlockCelebrationAudio() {
  const ctx = getCelebrationAudioContext();
  if (!ctx) return;
...
function playCelebrationSound() {
  ...
  try {
    ...
    const startSound = () => {
      ...
      // 3-second loud applause / crowd-cheer style sound, generated in-browser.
      const master = ctx.createGain();
      master.gain.setValueAtTime(0.0001, now);
      master.gain.exponentialRampToValueAtTime(0.95, now + 0.06);
      master.gain.setValueAtTime(0.95, now + 2.65);
      master.gain.exponentialRampToValueAtTime(0.0001, now + duration);
      master.connect(ctx.destination);

      const applauseFilter = ctx.createBiquadFilter();
      applauseFilter.type = "bandpass";
      applauseFilter.frequency.setValueAtTime(1800, now);
      applauseFilter.Q.setValueAtTime(0.85, now);
      applauseFilter.connect(master);

      const bodyFilter = ctx.createBiquadFilter();
      bodyFilter.type = "lowpass";
      bodyFilter.frequency.setValueAtTime(3200, now);
      bodyFilter.Q.setValueAtTime(0.4, now);
      bodyFilter.connect(master);

      function makeNoiseBurst(start, burstDuration, volume, filterTarget) {
        ...
        gain.gain.exponentialRampToValueAtTime(0.0001, start + burstDuration);

        source.connect(gain);
        gain.connect(filterTarget);
        source.start(start);
        source.stop(start + burstDuration + 0.02);
      }

      // Dense random claps. Many tiny filtered-noise bursts create applause.
      for (let t = 0; t < duration; t += 0.045) {
        const jitter = Math.random() * 0.035;
        const clapTime = now + t + jitter;
        const clapLength = 0.045 + Math.random() * 0.055;
        const clapVolume = 0.08 + Math.random() * 0.11;
        makeNoiseBurst(clapTime, clapLength, clapVolume, applauseFilter);
      }

      // A wider crowd wash underneath so it sounds fuller and louder.
      for (let t = 0; t < duration; t += 0.16) {
        const washTime = now + t + Math.random() * 0.08;
        makeNoiseBurst(washTime, 0.18 + Math.random() * 0.12, 0.045, bodyFilter);
      }

      // Bigger claps at the start and finish for a clear celebration hit.
      [0.02, 0.16, 0.31, 2.35, 2.55, 2.75].forEach(offset => {
        makeNoiseBurst(now + offset, 0.12, 0.22, applauseFilter);
      });
    };

    if (ctx.state === "suspended") {
      ctx.resume().then(startSound).catch((err) => {
        console.warn("Celebration sound needs one screen click first:", err);
      });
    } else {
      startSound();
    }
